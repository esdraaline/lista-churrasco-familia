...
Este projeto é acompanhado pelo Mentor de Projetos. Slug: `lista-churrasco-premium`. O estado executivo vive em `.project-mentor/project.yaml` e o histórico em `.project-mentor/sessions/`. **Nunca edite esses arquivos à mão**: toda escrita passa pelo CLI `mentor`.
...
2. Rode `mentor project lista-churrasco-premium --brief` e leia a última sessão em `.project-mentor/sessions/`.
...
4. Pergunte o objetivo só se não estiver claro. Depois rode `mentor start lista-churrasco-premium --objective "..." --agent <claude-code|codex|antigravity|copilot>`.
...
- Ação nova: `mentor action add lista-churrasco-premium --title "..."`. Concluir: `mentor action done lista-churrasco-premium <act-id> --level implemented|tested`. Bloqueio: `mentor blocker add lista-churrasco-premium --description "..."`.
...
2. Rode `mentor close lista-churrasco-premium --from <rascunho.md>` e mostre o resultado da validação. Se falhar, mostre o erro e **não** finja sucesso.
